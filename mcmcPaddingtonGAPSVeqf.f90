PROGRAM main
  ...
  DO j=1,Nstreams
     ...
     CALL thissampler(T,p,ydata,yNaN,Ny,DRAWstates(:,:,:,j),Nstates,Nx,DRAWsvol(:,:,:,j),Nsv,Ehbar0, Vhbar0, Eh0,sqrtVh0,minSV,maxSV, DRAWhvarbar(:,:,j), hvarbarT, hvarbarDof, DRAWhgap0(:,:,j), DRAWhSigma(:,:,j), NhSigma, hSigmaT, hSigmaDof, DRAWshockslopes(:,:,j), Nshockslopes, E0shockslopes, sqrtV0shockslopes, DRAWf(:,:,j), Nf, Ef0, sqrtVf0, DRAWmaxlambda(:,:,j), Nsim,Burnin, VSLstream,timer)
...
SUBROUTINE thissampler(T,p,y,yNaN,Ny,DRAWstates,Nstates,Nx,DRAWsvol,Nsv,Ehbar0, Vhbar0,Eh0,sqrtVh0,minSV,maxSV, DRAWhvarbar, hvarbarT, hvarbarDof, DRAWhgap0, DRAWhSigma, NhSigma, hSigmaT, hSigmaDof, DRAWshockslopes, Nshockslopes, E0shockslopes, sqrtV0shockslopes, DRAWf, Nf, Ef0, sqrtVf0, DRAWmaxlambda, Nsim, Burnin, VSLstream,timer)
  ...
  INTENT(IN)    :: T,p,y,yNaN,Ny,Nstates,Nx,Nsv,Ehbar0,Vhbar0,Eh0,sqrtVh0, hvarbarT, hvarbarDof, hSigmaT, hSigmaDof, NhSigma, minSV,maxSV,Nsim,Burnin, Nf, Ef0, sqrtVf0, Nshockslopes, E0shockslopes, sqrtV0shockslopes
  ...
  DOUBLE PRECISION :: gapdraw(-(p-1):T,Ny), gaplags(T,Ny * p), gapshocks(T,Ny)
  ...
  DOUBLE PRECISION, DIMENSION(T,Nsv)  :: transposeSV
  ...
  DO ! while j<= TotalSim
     ...
     IF (OK) THEN
        ...
        gapB = B(Nbar+1:Nbar+Ngap,2:1+Ny) ! assuming no cross correlation between trends and gaps
        call DTRTRI('l', 'u', Ngap, gapB, Ngap, status)
        if (status /= 0) then
           print *, 'DTRTRI error Bgap'
        end if
        ...
        gapshocks = gapdraw(1:T,:)
        do i=1,p
           gaplags(1:T,(i-1)*Ngap+1:i*Ngap) = gapdraw(1-i:T-i,:)
        end do
        ...
        call bayesVARXSVeqf(f, gapshocks, Ny, T, gaplags, Ngap * p, transposeSV, gapB, Ef0, iVf0, VSLstream) 
        ...
        xshock(Nbar+1:Nbar+Ngap,:) = transpose(gapshocks)
